...
import {
    ...
    History, X, Plus, CheckCircle, AlertTriangle
} from 'lucide-react';
import { saveToCloud, getFromCloud } from '../firebase';
...
const ImageToolPage = () => {
    ...
    const [isGuest, setIsGuest] = useState(true); // Assuming guest for now until auth context is fully wired
    ...
    useEffect(() => {
        const loadHistory = async () => {
            ...
            const history = await getFromCloud("images", userId);
    ...
    const handleSendMessage = (e) => {
        ...
        setTimeout(async () => {
            ...
            const newImage = {
                url: mockImageUrl,
                prompt: inputMessage,
                userId: "guest_user", // Replace with real ID
                timestamp: new Date()
            };
            await saveToCloud("images", newImage);
        ...
        <div className="home-page">
            ...
            <nav className="navbar glass-panel">
                ...
                <div className="nav-actions">
                    ...
                    <button className="nav-btn" title="שמירה בענן">
                        <Cloud size={20} />
                    </button>
            ...
            <div className="dashboard-container" style={{ flexDirection: 'row-reverse' }}>
                {/* Preview Area (Left Side now due to reverse) */}
                <div className="preview-area">
                    ...
                    <div className="preview-content" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                        {isThinking && !showAd ? (
                            ...
                            // Empty State with Blob Animation or Ad Placeholder
            ...
            {showGallery && (
                <div className="modal-overlay" onClick={() => setShowGallery(false)}>
                    <div className="modal-content" style={{ maxWidth: '800px', width: '90%', maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
                        ...
                        <h3>הגלריה שלי</h3>
                        {isGuest && <div className="alert-warning" style={{ background: '#fffbeb', color: '#92400e', padding: '0.5rem', borderRadius: '4px', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            <AlertTriangle size={16} />
                            שים לב: במצב אורח התמונות נשמרות באופן זמני בלבד.
                        </div>}

                        <div className="gallery-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '1rem' }}>
                            {galleryImages.length === 0 ? (
                                <p>עדיין לא יצרת תמונות.</p>
                            ) : (
